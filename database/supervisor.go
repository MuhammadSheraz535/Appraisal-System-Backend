...
func (sc *SupervisorService) GetSupervisorsWithQuery(name string) ([]models.Employee, error) {
...
func (sc *SupervisorService) GetSupervisorByIdDB(id string) (*models.Employee, error) {
	// Query the employees table for an employee with the specified ID and role "supervisor"
	var supervisor models.Employee
	if err := sc.db.Table("employees").Where("id = ? AND role = ?", id, supervisorRoleName).First(&supervisor).Error; err != nil {
		return nil, err
	}
...
func (sc *SupervisorService) UpdateSupervisorInDatabase(supervisorId string, req models.Supervisor) error {
	// Query the employees table for an employee with the specified ID and role "supervisor"
	var supervisor models.Employee
...
func (sc *SupervisorService) DeleteSupervisorFromDB(supervisorId string) error {
